...
class ItemForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = { xcolor: "", ycolor: "", items: [] };
  }
...
const ItemMap = (props) => {
  ...
  const itemList = props.items.map((item, id) => {
    return <ItemDisplay key={id} item={item} id={id} {...other} />;
  });
  return itemList;
};
  ...
  if (props.item.editing) {
    return (
      <PanelBlock type="anchor" style={style}>
        <p>{props.item.id}:</p>
        <Input
          changeEvent={props.handleFieldUpdate(props.id, "x")}
        ...
        />
        <Input
          changeEvent={props.handleFieldUpdate(props.id, "y")}
          cls="is-small item-form"
          placeholder="height"
          value={props.item.y}
          type="text"
        ...
        />
        <Button
          clickEvent={props.handleDeleteItem(props.id)}
          cls="is-small"
          style={{ float: "right" }}
          text="☓"
        />
